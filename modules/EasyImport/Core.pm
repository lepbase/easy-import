		...
		for (my $g = 0; $g < @gene; $g++){
          ...
          if ($cds && $params->{'MODIFY'}{'AUTO_PHASE'}){
            ...
            if ($len % 3 != 0 || $phases[0] != 0){
              ...
              $l = $mrna->attributes->{_strand} == 0 ? $phases[0] : $phases[-1];
          ...
          elsif ($cds && $params->{'MODIFY'}{'INVERT_PHASE'}){
						for (my $p = 0; $p < @phases; $p++){
	...
	foreach my $key (keys %{$params->{'GFF'}}){
        ...
        if ($value[1]){
          ...
          $gff->override({$value[0] => {}});
			...
			#elsif ($type eq 'COL_NINE'){
			#	$gff->col_nine($value[0]);
			#}
...
sub setup_core_db {
	...
	my $sth_tax = $dbh_tax->prepare("SELECT names.taxon_id, names.name FROM ncbi_taxa_name AS names JOIN ncbi_taxa_node AS nodes ON names.taxon_id = nodes.parent_id WHERE nodes.taxon_id = ? limit 1");
